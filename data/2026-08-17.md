...
TL;DR: 本文针对异构移动代理网络（含不可控任务代理和可控通信代理）中的在线重定位问题，指出经典Fiedler梯度控制器依赖Fiedler向量（拉普拉斯矩阵第二小特征值对应特征向量）而难以分布式估计，提出一种结构分解视角，将控制器拆分为局部交互规则和图嵌入组件，并据此提出A-Fiedler方法——用邻接矩阵主特征向量替代Fiedler向量作为图嵌入。实验表明A-Fiedler在无通信约束时性能与传统方法相当，在分布式估计下鲁棒性更优。


<details>
  <summary>Details</summary>
Motivation: 吞吐量目标不适合实时控制，常采用代数连通性等谱图指标作为替代目标。但控制代数连通性依赖Fiedler向量，其分布式估计需要无限轮通信才能收敛，这在局部通信约束下不切实际。

Method: 首先对Fiedler梯度控制器进行结构分解，识别出局部交互规则和图嵌入两个组件；然后提出A-Fiedler方法，用邻接矩阵的主导特征向量（常用于节点到潜在几何空间的图嵌入）替代Fiedler向量作为图嵌入，从而更自然地适配分布式实现。

Result: 对比实验显示：在无通信约束下A-Fiedler与传统Fiedler梯度控制器性能相当；在分布式估计下，A-Fiedler鲁棒性更好。例如在相同通信轮数下，Fiedler梯度控制器可能收敛到不连通配置，而A-Fiedler能保持性能。

Conclusion: A-Fiedler为分布式网络控制提供了更简单的路径，在保持性能的同时显著提升了分布式实现下的鲁棒性。
...
TL;DR: 本文提出hint²方法，利用分层世界模型在推理时引导短视策略满足复杂线性时序逻辑（LTL）规范。高层模型预测任务相关原子命题的动作诱导转移以引导LTL自动机进度，低层动力学模型预测即时状态演化以进行局部安全引导。在CALVIN基准上优于现有推理时引导方法，并成功在真实UR5e机械臂上完成任务。


<details>
  <summary>Details</summary>
Motivation: 大语言条件策略在运行时执行丰富指令方面取得进展，但仍难以处理时间结构（长期依赖）和安全约束。尽管LTL能表达复杂的非马尔可夫指令，但现代策略生成短时域动作块并以闭环方式重规划，而几乎所有LTL规范都是在长时域轨迹上评估的，导致两者之间存在本质不匹配。

Method: hint²利用分层世界模型在推理时引导短视策略。该方法从每个世界模型的抽象层级导出两个独立的引导目标：1) 高层模型预测未来动作诱导的任务相关原子命题转移，用于引导LTL自动机的进度；2) 低层动力学模型预测即时状态演化，用于提供精确的局部安全引导。

Result: hint²克服了现有LTL引导扩散方法的局限性，在CALVIN基准上优于现有推理时引导方法，能够比语言条件基线更优雅地完成包含复杂活性（liveness）和安全约束的指令，并在真实UR5e机械臂上验证了复杂指令处理能力。

Conclusion: 通过分层抽象分别处理长期任务进展和短期安全约束，hint²有效弥合了短视策略与长时域LTL规范之间的鸿沟，为推理时引导操作策略满足复杂时序逻辑提供了可行的解决方案。
...
TL;DR: 提出Graph-MambaNav，一种基于目标感知的时空图编码框架，通过LLM引导的节点排序（目标相关对象延迟处理以聚合更丰富上下文）和Mamba-based选择性扫描，解决目标导航中对象关系推理和长距离依赖问题，在AI2-THOR、RoboTHOR及真实机器人上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有图方法虽在特征或注意力层面融入目标感知，但保持排列不变性，缺乏对信息传播顺序的显式控制，难以建模目标依赖的重要性和长距离依赖。Graph-Mamba表明节点排序对全局推理至关重要，因此需要将节点优先级机制引入目标导航。

Method: 提出Graph-MambaNav框架：(1) 基于LLM常识对象关系初始化节点排序和边权重，按对象与目标的相关性进行启发式排序，使信息量更大的对象更晚处理以聚合更丰富上下文；(2) 空间模块结合局部消息传递与GraphMamba-based全局选择性扫描；(3) 时间模块采用Mamba-based序列建模，在对象级时间顺序上进行选择性历史上下文聚合以支持长距离时序推理。

Result: 在AI2-THOR和RoboTHOR基准上取得改进的导航性能与泛化能力，真实机器人部署进一步验证了方法的有效性。

Conclusion: 通过显式控制节点处理顺序（基于LLM常识先验的目标相关性排序），并融合Mamba选择性扫描进行全局时空推理，Graph-MambaNav有效克服了传统图方法排列不变性的局限，为目标导航提供了可泛化的结构化推理方案。
...
TL;DR: 本文提出知识-数据双驱动强化学习（KDDRL）框架，解决混合交通中自动驾驶决策面临的意图不确定性、非平稳性及混合动作空间三大挑战，通过条件深度生成模型合成意图感知的未来轨迹，结合知识-数据双驱动范式与耦合模块实现连续跟车与离散换道的异步多时间尺度优化，在安全、效率和舒适性上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 混合交通中自动驾驶决策面临三个相互关联的挑战：（1）基于物理先验的强化学习模型无法捕捉潜在交互意图和多样化驾驶行为，限制了主动推理能力；（2）周围车辆的突然变道导致非平稳性，长尾安全事件未被充分探索；（3）连续跟车与离散换道具有不同时间尺度，混合动作空间使统一RL训练不稳定。

Method: 提出KDDRL框架，包含三个核心组件：（1）条件深度生成模型合成意图感知的未来轨迹，将被动感知转化为主动预测状态；（2）知识-数据双驱动范式在这些预测状态上运行，融合概率数据驱动洞察与物理约束以引导安全探索；（3）耦合模块将意图感知轨迹和物理约束压缩为紧凑的共享嵌入，实现连续跟车与离散换道的异步多时间尺度优化并保持互信息。

Result: 在数据集校准的仿真中评估表明，KDDRL有效处理意图不确定性、加速训练收敛，并在安全性、效率和舒适性方面优于传统基线方法。

Conclusion: KDDRL通过双驱动范式和统一表示解决了混合交通中自动驾驶决策的意图建模、非平稳性及混合动作空间等关键挑战，为复杂交互场景下的安全决策提供了有效方案。
...
TL;DR: 本文提出Onto-EV-WM，一个基于本体论（ontology）的诊断与验证门控纠正接口，叠加在EV-WM之上而非替代其世界模型架构。它通过任务局部TBox定义实体类型和谓词签名，用确定性规则记录缺失谓词及其参数并分配路由标签，通过谓词门控的验证协议决定是否重试。在PointMaze、LIBERO-Goal和LIBERO-Plus基准上报告了88%-94%的成功率。


<details>
  <summary>Details</summary>
Motivation: EV-WM虽然用特征分数和事件分数表示候选质量，但未显式记录未满足的任务谓词、可用纠正机制的路由标签，以及纠正后的接受结果。缺乏这些显式信息导致诊断不透明、纠正不可追溯，因此需要一个更高层接口来补足这些语义信息。

Method: Onto-EV-WM由三个组件构成：(1) 任务局部TBox定义实体类型、谓词签名和约束；(2) 源特定grounding映射将预测或模拟器观测状态转换为任务ABox；(3) 确定性规则在分配路由标签时保留每个缺失谓词及其参数。学习型或启发式proposer与该符号接口分离；原生任务谓词决定接受，有界协议决定验证失败时是否重试。

Result: 在PointMaze评估中，EV-WM和Onto-EV-WM均报告94%成功率，平均终态距离分别为0.90573和0.61177，单独预算的搜索达100%。在LIBERO-Goal上，完整配置在种子0上报告93.8%纠正窗口成功率，四个评估采样种子上为94.05±0.30%。在固定的10,030任务LIBERO-Plus注册表上，Onto-EV-WM成功8,526个任务（85.00%），LIBERO-10为65.98%，LIBERO-Goal为91.39%，LIBERO-Object和LIBERO-Spatial均为91.38%。

Conclusion: 这些数字报告了完整本体grounded配置在测试模拟器协议下的性能；本体单独的因果贡献未被单独测量，真实机器人恢复也未评估。Onto-EV-WM通过在EV-WM之上增加显式的语义诊断与验证门控层，改善了纠正的透明度与可追溯性。
...
TL;DR: 本文提出一种受大脑双侧半球组织启发的模块化循环控制器，由两个GRU模块通过可学习的延迟半球间通道连接，在可微分的双臂肌肉骨骼模拟器中端到端训练。实验表明该模块化架构在到达和保持任务中显著优于同等容量的单体架构，且学习到的半球间通信可通过调节肌肉共收缩来改善末端精度并降低能量消耗。


<details>
  <summary>Details</summary>
Motivation: 肌肉骨骼系统中的机器人运动控制需要在信号依赖噪声和能量成本约束下同时实现快速准确运动和鲁棒姿态稳定。模块化控制器有望通过子模块间交互来分配这些竞争性需求，但模块化架构是否在现实约束下优于单体架构、以及模块间通信如何影响控制策略尚不清楚。

Method: 采用受大脑双侧半球组织启发的架构：一个由两个GRU模块组成的循环控制器，通过可学习的延迟半球间通道连接。该控制器在可微分的双臂肌肉骨骼模拟器中端到端训练，并在到达和保持任务中与容量匹配的单体基线以及无通信的模块化控制器进行对比。

Result: 模块化架构在到达和保持任务中显著优于容量匹配的单体基线。与无通信的模块化控制器相比，学习到的半球间通信改进了末端精度、在非零延迟条件下降低了能量成本，并减少了肌肉共收缩。

Conclusion: 生物启发的模块化控制器为机器人在噪声和能量约束下的鲁棒运动控制提供了实用途径，模块间通信提供了一种在精度、稳定性和驱动成本之间调节权衡的机制。
...
TL;DR: BICPO-VLA通过指令感知的因果历史编码、序贯Haar子空间生成和参考相对Flow-DPO三个步骤，依次解决机器人请求到手部交接（request-to-handoff）间隙中的行为歧义、物理状态漂移和残余不兼容三个耦合问题。


<details>
  <summary>Details</summary>
Motivation: 请求到手部交接间隙存在三个耦合来源：请求时的行为意图歧义、动作生成过程中的物理状态漂移、以及新动作最终接管控制时的残余不兼容性。现有方法未系统性地解决这些耦合问题。

Method: BICPO-VLA按顺序处理三个问题：1) 指令感知的因果历史编码器识别指令和任务进度所支持的行为；2) 序贯Haar子空间生成将每个动作块分解为互补的成对脚手架和残差系数，通过两个专门的生成阶段后精确重构，减少原始动作空间的迭代细化，缩短机器人在新动作块可用前继续移动的时间间隔；3) 将已知的离场动作滚动到实际交接状态，并在行为匹配的候选中应用参考相对Flow-DPO，使生成的块适应剩余的交接不匹配而不改变其意图行为。

Result: 通过缩短原动作空间中的迭代细化过程，减少了新动作块可用前机器人继续移动的时间间隔，并通过参考相对Flow-DPO在行为匹配候选中实现动作适配，从而缓解交接不匹配。

Conclusion: BICPO-VLA通过解耦三个耦合源并逐一解决，实现更平滑、更准确的机器人动作交接，在保持行为意图的同时减少交接间隙。
...
TL;DR: 本文提出AdvDex，一个统一的视觉-语言-动作框架，通过大规模人类操作演示数据集OmniShare、规范的联合对齐动作空间（JAAS）和域对抗学习，实现跨形态的灵巧操作学习与零样本人-机器人技能迁移。


<details>
  <summary>Details</summary>
Motivation: 灵巧操作是具身智能的基础能力，但规模化仍面临两大挑战：机器人演示数据收集成本高昂，且不同机器人形态的动作空间各异；此外，在异构数据上训练的策略会将与任务相关的视觉线索与形态特定的外观特征纠缠在一起，限制跨形态泛化能力。

Method: AdvDex包含三个核心组件：(1) OmniShare，一个大规模多模态人类操作演示数据集，提供高质量运动学监督和触觉测量，减少对机器人遥操作的依赖；(2) 联合对齐动作空间（JAAS），一种规范动作表示，包含SE(3)腕部姿态和15个手指关节，从而在功能上对齐人类手、灵巧机器手和平行夹爪；(3) 域对抗学习，用于减少所学视觉表示中的形态特定信息。

Result: 在手部动作预测和真实世界灵巧操作实验中，AdvDex相比基线方法取得了一致的性能提升，实现了有效的零样本人到机器人技能迁移、对未见物体和环境的泛化，以及数据高效的少样本适应。

Conclusion: AdvDex通过结合大规模人类演示数据、规范动作空间和域对抗视觉学习，有效解决了灵巧操作跨形态泛化的核心问题，为具身智能的灵巧操作规模化提供了可行路径。
...
TL;DR: 本文展示了一种平面航天器模拟机器人ATMOS（多功能轨道系统自主测试平台），用于在微重力类似环境中进行硬件在环的制导与控制策略评估，并通过首尔至斯德哥尔摩的洲际远程控制实验验证了结合状态预测与轨迹跟踪的对接控制策略在随机通信延迟下的有效性。


<details>
  <summary>Details</summary>
Motivation: 在轨服务与空间交会对接任务中，地面操作员与航天器之间存在显著的往返通信延迟（尤其在洲际距离下），这给遥操作控制带来了挑战。需要一个成本低廉、可靠且快速的物理测试平台来验证微重力类似环境下配备通信延迟的遥操作控制架构。

Method: 利用ATMOS平面气浮式航天器模拟机器人作为硬件测试平台，设计并实验验证了一种将状态预测与轨迹跟踪控制相结合的控制策略，用于在时变随机通信延迟条件下完成对接机动。实验设计包括首尔与斯德哥尔摩之间的长距离远程控制实验，以引入真实的洲际延迟和可变性。

Result: 实验结果表明，所提出的控制策略能够在随机时变通信延迟下成功完成对接操作，同时证明了ATMOS平台能够支持快速、可靠且成本效益高的航天器遥操作概念验证。

Conclusion: 本研究为在微重力类似环境中鲁棒验证在轨操作控制方案奠定了第一步基础，展示了ATMOS作为地面测试平台在航天器遥操作验证中的实用价值与潜力。
...
TL;DR: 本文提出ART（Agentic Robot with Tool-use）框架，将端到端视觉-语言-动作（VLA）模型与智能体工具使用相结合，通过工具注入机制降低动作解空间的复杂度，从而在显著减少数据依赖的同时提升跨任务泛化能力。实验表明ART在模拟和真实世界任务中比主流基线方法成功率高出20%。


<details>
  <summary>Details</summary>
Motivation: 传统的端到端VLA模型面临两大挑战：一是动作解空间过于庞大复杂，导致泛化能力受限；二是对大规模训练数据的强依赖，数据获取成本高昂。作者希望通过引入工具使用机制来降低动作空间的复杂性，从而同时解决泛化性和数据依赖两个核心问题。

Method: ART是一种工具注入框架，可对任意VLA模型进行微调，使其能够利用现成的工具模块，涵盖三个层面：底层视觉感知工具、高层可供性（affordance）推理工具和具身能力增强工具。作者构建了仅30K条工具使用轨迹和动作演示的小规模数据集，并设计了适用于挑战性环境中长轨迹工具使用推理的训练方案。

Result: 在模拟环境和真实世界任务（如黑暗中从新视角进行抓取放置）中，ART相比主流基线方法实现了20%更高的成功率。实验还表明，模块化工具利用带来了更高效的训练、轻量化的部署以及新工具的可扩展集成。

Conclusion: 基于智能体的工具使用方法为VLA系统带来了鲁棒性、适应性和可扩展性方面的显著优势，为VLA系统在复杂现实场景中的实际部署铺平了道路。
...
TL;DR: 本文提出一个将操作解耦为策略生成器和动作执行模块的统一的平面物体机器人操作框架，并引入FlatLab仿真基准来系统评估方法在未见物体和类别上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 平面物体的机器人操作因不可抓取构型以及物体几何和材质差异大而具有挑战性。现有方法依赖启发式预操作，且在封闭环境中评估、泛化能力有限。

Method: 将操作解耦为两个模块：1) 策略生成器，通过模拟数据变换和对比学习学习策略中心、物体不变的表示，从物体点云预测合适的操作策略；2) 动作执行模块，根据预测策略将长时程操作分解为可复用的动作基元，并动态组合生成稳定轨迹。同时提出FlatLab仿真基准，提供高保真物理仿真、自动化多模态数据采集、标准化任务定义和评估协议。

Result: 在FlatLab中的实验表明，所提方法能有效泛化到未见物体和类别，性能优于现有基线方法。

Conclusion: 通过在FlatLab上的系统评估验证了所提出解耦框架在平面物体操作中具有良好的泛化能力，为这一挑战性任务提供了新的统一解决方案和标准化评估基准。
...
TL;DR: 本文提出PILOT框架，一种基于约束感知的特权模仿学习框架，用于视觉端到端UAV运动规划。它通过双目标损失函数将最优控制专家的规划策略蒸馏到学生策略中，并结合TCN时空感知融合模块缓解部分可观测性问题。实验表明，PILOT在四旋翼和固定翼仿真中达到与特权专家相当的性能，计算开销降低80%以上，并成功实现了室内外零样本部署。


<details>
  <summary>Details</summary>
Motivation: 杂乱环境中的自主导航受到部分可观测性和动态约束的阻碍。给定历史深度图像和里程计信息，视觉端到端UAV运动规划需要在资源受限平台上实现实时、安全且动态可行的轨迹生成，同时克服瞬时视野（FOV）的限制和计算开销过大的问题。

Method: PILOT框架包含三个核心组件：(1) 双目标损失函数，将计算密集型最优控制专家的规划策略蒸馏到学生策略中，同时正则化安全和动态要求；(2) 基于时序卷积网络（TCN）的时空感知融合模块，整合历史深度图像和里程计以推断任务相关的潜在上下文，增强超出瞬时FOV的空间感知能力；(3) 轨迹参数化层，将网络输出映射为结构化轨迹，在训练中显式施加连续性、动态一致性和障碍物软惩罚约束。

Result: 在四旋翼和固定翼飞机的仿真中，PILOT实现了与特权专家相当的性能，同时将计算开销降低了80%以上。室内和室外零样本部署的成功验证了该规划器的实际可行性和跨域泛化能力。

Conclusion: PILOT通过特权模仿学习与约束感知正则化的结合，有效解决了部分可观测下视觉UAV运动规划的挑战，在不维持持久地图记忆的前提下实现了与最优专家媲美的规划质量，并通过零样本部署展示了良好的跨域泛化能力。尽管无法为未见观测提供正式的安全保证，该框架在计算效率和部署可行性方面具有显著优势。
...
TL;DR: AgilePE是一个基于自博弈强化学习的完整无人机追捕-逃逸系统，实现了从敏捷底层控制到竞争策略优化再到仿真到现实部署的统一框架。策略直接将机载状态观测映射为集体推力和机体角速度指令，通过优先级虚构自我对弈（PFSP）和多样化对手池进行训练，最终实现零样本迁移到真实四旋翼无人机上。


<details>
  <summary>Details</summary>
Motivation: 自主追捕-逃逸是无人机面临的基础挑战，需要在强耦合动力学和不断变化的对手行为下快速决策。传统基于规则或微分博弈的方法难以应对高维空中交互和敏捷机动，因此需要一种端到端的学习方法。

Method: 采用自博弈强化学习框架：1）策略直接映射机载状态观测到集体推力和机体角速率（CTBR）命令，实现无中间轨迹规划器的端到端敏捷机动；2）使用优先级虚构自我对弈（PFSP）和多样化对手池进行竞争性策略优化，以稳定训练并减少策略振荡；3）构建硬件对齐的仿真管线，建模执行器响应动力学、通信延迟和领域随机化，实现零样本转移到真实四旋翼。

Result: 学到的策略无需任务特定微调即可零样本迁移到真实四旋翼上。真实世界实验复现了仿真中观察到的追捕-逃逸策略，包括快速闪避和侧翼包抄等战术，并展示了交互式双智能体零样本部署。

Conclusion: AgilePE证明了自博弈强化学习配合硬件对齐的仿真管线能够使无人机在追捕-逃逸任务中涌现出复杂的敏捷战术，且策略具备良好的仿真到现实迁移能力。
...
### [14] [Vibration Suppression in Collaborative Flexible Payload Manipulation Using Passive Force Control](https://arxiv.org/abs/2608.14244)
*Alaa Abderrahim,Antonio Rosales,Ferdinando Milella,Markku Suomalainen,Shuai Li*

Main category: cs.RO

TL;DR: 本文提出一种基于主从（leader-follower）配置的双工业机械臂协作载荷操控策略，用于抑制大型柔性载荷在运动过程中的横向振动。领导者机器人产生整形速度指令引导运动，跟随者机器人通过导纳控制器对外部力保持柔顺，从而实现重型和大型柔性物体的协作操控。


<details>
  <summary>Details</summary>
Motivation: 在大型重型结构中，运动过程中产生的振动对精确操控构成重大挑战。特别是在未来聚变反应堆（托卡马克）远程维护等前沿项目中，柔性载荷的可靠操控被视为关键任务。现有方法难以实现对更重、更大的柔性物体进行协作操控，且面临振动抑制和异构机器人规格差异等额外挑战。

Method: 采用主从（leader-follower）配置的双工业机械臂协作操控方案。领导者机器人通过整形速度命令引导运动，跟随者机器人通过导纳控制器根据领导者施加的外部力估计值实现柔顺响应。系统动力学采用等效质量-弹簧-阻尼模型进行建模，并证明在适当的导纳参数下系统总能量被动耗散，同时提供了稳定性证明。

Result: 数值仿真验证了所提出的方法，实验结果证明了其有效性。该方法能够抑制柔性载荷运动过程中的横向振动，并实现异构机器人协作操控更重、更大的柔性物体。

Conclusion: 所提出的主从协作载荷操控策略能够有效抑制柔性载荷的横向振动，通过被动耗散系统能量保证稳定性，为大型柔性结构（如托卡马克远程维护）的精确操控提供了可行的解决方案。
...
TL;DR: 本文提出OccPlanner，一种基于占用条件扩散的像素目标导航规划器，将像素级目标从相机视图锚定到自车度量空间，并通过时序视觉上下文和局部3D占用特征逐步条件化目标表示。同时提出L3ROcc来自动标注单目RGB导航视频为机器人中心局部3D占用，实现大规模监督。在5-8米距离范围内，OccPlanner将平均成功率从NavDP的20.81%提升至71.55%，并在真实Unitree Go2机器人上验证了sim-to-real迁移能力。


<details>
  <summary>Details</summary>
Motivation: 像素目标导航中，目标像素缺乏度量深度和可通行性信息，导致3D目标接地和碰撞规避的连续规划面临挑战。现有方法难以将像素级意图转化为机器人可执行的度量空间规划。

Method: OccPlanner：目标感知的占用条件扩散规划器，将像素目标接地到自车度量空间，并依次在时序视觉上下文和学习到的局部3D占用特征上条件化目标表示。L3ROcc：通过几何重建和射线可见性推理，将单目RGB导航视频转换为机器人中心的局部3D占用标注。

Result: 在InternScenes的四个未见场景类别和两个目标距离范围的闭环仿真中，5-8米设置下平均成功率（SR）从NavDP的20.81%提升至71.55%；在cluttered-easy和cluttered-hard场景分别达到86.20%和84.92%。真实Unitree Go2上的开环实验初步验证了sim-to-real迁移及L3ROcc监督的适应性。

Conclusion: OccPlanner通过占用条件扩散与L3ROcc大规模占用监督相结合，有效解决了像素目标导航中的3D接地与可通行性建模问题，显著提升导航成功率，并展现跨域泛化与真实机器人部署潜力。
...
### [16] [Spatiotemporal Tube-Based Safety-Certificate for Autonomous Navigation of Articulated Vehicles](https://arxiv.org/abs/2608.14531)
*Mohd. Faizuddin Faruqui,Ratnangshu Das,Ravi Kumar L,Pushpak Jagtap*

Main category: cs.RO

TL;DR: 本文提出一种基于时空管道的规划方法，用于铰接式车辆（如牵引半挂车、卡车拖车、牵引AGV和道路列车）的自动驾驶导航，确保牵引车及其拖挂的系列挂车始终保持在道路走廊内。通过在路径安全证书中融入运动学和摆动约束，该方法在复杂路线的卡车-拖车导航仿真中得到了验证。


<details>
  <summary>Details</summary>
Motivation: 铰接式车辆是货运的主力工具，但其自动驾驶导航极具挑战性。这类车辆由于物理特性和运动约束，在狭窄路线上机动时面临显著困难。现有方法难以确保牵引车后面的系列拖车不偏离道路走廊，因此需要一种能够为整个车辆组合提供安全规划的方法。

Method: 该方法采用时空管道（spatiotemporal tube）规划方式，针对铰接式车辆中的每个链接元素利用其运动学特性，并结合摆动（sway）约束来构建安全的管道空间。通过考虑容许修正（permissible corrections）对时空管道进行修正，为车辆在给定路线上提供路径安全证书，从而确保牵引车及其拖曳的系列拖车始终保持在道路边界之内。

Result: 所提出的规划方法在复杂路线的卡车-拖车导航仿真中得到了验证，证明了该方法能够为铰接式车辆提供具有认证的路径规划，保证整个车辆组合（包括被牵引的系列拖车）始终保持在道路走廊内。

Conclusion: 该研究为铰接式车辆的自动驾驶导航提供了一种具备安全保证的时空管道规划方法，通过运动学建模、摆动约束和容许修正相结合，实现了对拖挂车辆组合的全程路线安全认证，适用于多种铰接式车辆类型。
...
TL;DR: 本文提出MMUSV-Sim，一个基于Unreal Engine 5和Project AirSim的多无人艇（USV）协同感知仿真与数据生成平台，支持岛屿、开放海域、港口等场景，可配置天气、时段、波浪条件，并提供多样化船舶资产库和样条多艇运动。平台可跨多USV采集RGB、深度、语义、LiDAR和雷达观测，并导出共享世界状态的逐智能体标注。实验验证了波浪设置对船舶垂荡、横摇、纵摇的影响，以及在LiDAR协同BEV船舶检测中，早期融合的AP@0.5达到72.74，远高于单艇的45.54。


<details>
  <summary>Details</summary>
Motivation: 多无人艇协同感知能够融合互补观测信息，扩展海上目标感知的范围与视野，但大规模开发此类系统需要统一的、可配置的多USV场景工作流以及多模态采集和共享标注平台，当前尚缺乏此类工具。

Method: 基于Unreal Engine 5和Project AirSim构建MMUSV-Sim仿真平台，提供岛屿、开放海域、港口环境，支持可配置天气/时段/波浪条件、多样化船舶资产库和样条曲线驱动的多艇运动；平台在多个USV上同时获取RGB、深度、语义、LiDAR与雷达观测，并捕获统一世界状态以进行逐智能体标注导出。

Result: 实验验证了波浪设置能产生预期的船舶垂荡、横摇和纵摇变化，并评估了投影标注与语义渲染的几何一致性。在生成的多USV数据集上，基于LiDAR的协同BEV船舶检测中，早期融合的AP@0.5达72.74，显著高于单USV的45.54。

Conclusion: MMUSV-Sim为多USV协同感知提供了可扩展的仿真与数据生成基础设施，实验表明协同检测相比单艇检测有显著性能提升，验证了平台的有效性与实用性。
...
TL;DR: 本文提出首个全并行计算框架来加速大规模测绘中的LiDAR光束法平差，通过自适应异步数据加载、自底向上的体素化特征提取和基于majorization-minimization的并行优化求解，在保持与最先进方法相当的建图精度下实现了最高10倍的计算效率提升。


<details>
  <summary>Details</summary>
Motivation: LiDAR光束法平差在大规模建图中构建全局一致的点云地图，但计算密集且耗时，现有方法难以在GPU上充分利用并行计算能力。需要一种全并行框架来加速该过程，以处理大规模点云数据集的效率问题。

Method: 本文方法包含三个关键技术：(1) 自适应异步数据加载策略，在内存受限的GPU上高效处理大规模点云数据；(2) 新颖的自底向上体素化方法提取平面特征，实现全并行预处理；(3) 基于majorization-minimization公式加速优化中的计算密集型任务，包括残差、Jacobian和Hessian矩阵的计算以及并行增量求解器。同时提供了时间复杂度分析。

Result: 在多种计算平台上的大规模公开数据集上进行广泛基准测试，验证了方法的鲁棒性和适应性，计算效率最高提升10倍，同时建图精度与最先进方法相当。

Conclusion: 本文提出的全并行LiDAR光束法平差框架在保证建图精度的前提下显著提升了计算效率，代码已开源供后续研究使用。
...
TL;DR: 本文提出PRM-as-a-Judge 1.5，一个用于机器人过程评估的工具包，将部署视频转化为密集进度曲线并派生多种细粒度指标，引入三个新指标刻画失败侧进展、回撤后恢复和成功侧执行质量，同时推出RoboPulse++用于评估过程奖励模型的可靠性，并发布完整的评估套件。


<details>
  <summary>Details</summary>
Motivation: 细粒度机器人评估对于理解具身模型至关重要，但现有评估方法仅依赖二元成功率或基于规则的流程评分，无法深入揭示模型能力，需要更精细、更透明、可复现的评估手段。

Method: 将机器人部署视频转换为密集进度曲线，并从中派生多个细粒度指标；在版本1.0基础上引入三个新指标，分别表征失败侧进展、回撤后恢复能力和成功侧执行质量；基于基准数据集的部署视频对具身模型进行全面评估；引入RoboPulse++评估过程奖励模型的可靠性；发布包含基准、指标实现和可视化工具的完整评估套件。

Result: 对多个具身模型进行了综合细粒度评估，提供了细粒度指标结果和关键发现；RoboPulse++为过程奖励模型评估提供了更准确的测试平台。
...
TL;DR: 本文提出CORAL，一种结合五阶段课程学习与阶段感知奖励的强化学习方法，用于城市自动驾驶的长视距目标导向导航。在CARLA环境中使用PPO训练多流actor-critic网络，在最长路线和全部行为约束下达到100%成功率（基线仅5%和10%），并零样本迁移到七个未见城镇（68-98%成功率）。


<details>
  <summary>Details</summary>
Motivation: 强化学习在城市自动驾驶中前景广阔，但长视距目标导向导航要求策略同时习得多重竞争行为（到达远端目标、跟踪路线、避开障碍、遵守信号灯），而固定目标无法确定学习这些行为的顺序。因此需要一种机制来引导策略按合理顺序逐步习得这些能力。

Method: CORAL同步推进两种调度：(1) 五阶段课程学习，逐步延长路线并收紧行为约束；(2) 阶段感知奖励，其组件权重随任务难度从任务进度转向路线跟随、安全性、平滑性和规则合规。策略为多流actor-critic网络，采用PPO在CARLA中训练，输入为99维紧凑状态（极坐标LiDAR直方图、车辆遥测、自车框架路线几何、交通规则指示），无需点云编码器或BEV栅格化。

Result: 在相同的评估协议下，CORAL在所有20个最长路线评估回合中均到达目标（100%成功率），而两个PPO基线的成功率仅5%和10%。因子消融实验表明两种调度单独使用均不及组合使用：去除任一调度会降低成功率和路线完成度，同时禁用则成功率降至55%。训练于单一城镇后，零样本迁移到七个未见城镇，在100-150米路线上的成功率为68-98%，平均横向偏差低于0.35米。

Conclusion: 课程学习与阶段感知奖励的组合能有效解决长视距城市驾驶中多重行为竞争学习的排序问题，显著超越固定目标的PPO基线，并展现出良好的跨城镇泛化能力。
...
TL;DR: 本文提出ReflexBench基准测试和ReflexVLA模型，用于解决现有VLA模型在动态交互（反应关键型）操作任务上评估不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型基准主要评估静态操作任务的泛化能力，忽略了动态交互场景。真实机器人操作中常常需要快速反应（如接住掉落物体、避开移动障碍），这类反应关键型操作对模型的低延迟感知和时序推理能力有更高要求，但缺乏专门的基准和高效模型。

Method: 1）提出ReflexBench基准，包含六个动态任务，并引入将模拟器步进与机器人控制解耦的评估框架，支持同步和异步推理下的可配置延迟设置。2）提出ReflexVLA模型，通过潜在未来预测和多帧时序融合增强视觉骨干网络的时序推理能力，并通过批处理视觉编码和CUDA图重放降低部署延迟，无需大规模机器人数据预训练。

Result: 实验表明ReflexVLA在动态操作性能上持续改进，同时在标准静态操作基准上保持有竞争力的精度；真实世界实验进一步验证了其在实际部署条件下的有效性。

Conclusion: ReflexBench填补了反应关键型操作基准的空白，ReflexVLA证明了通过时序推理增强和延迟优化，VLA模型可以在不依赖大规模机器人数据预训练的情况下高效处理动态操作任务。
...
TL;DR: 本文提出一种基于银包纱与PDMS涂层的纺织集成电容传感平台，通过一、二、四层加捻结构系统研究了纱线层级架构对电容传感特性的影响。四层结构在灵敏度（0.1331 MPa⁻¹@100 kHz）、机械强度（断裂伸长率85%、最大载荷89.7 N）、低迟滞、热稳定性和15000次循环稳定性上表现最优，同时实现了40 mm近距离检测和4×4传感阵列空间映射，并展示了403 ms端到端延迟的机器人实时感知应用。


<details>
  <summary>Details</summary>
Motivation: 纺织集成电容传感器虽已广泛应用，但纱线层级架构对电容换能特性的影响尚未被系统量化。本研究旨在填补这一空白，将纱线构型确立为可调节的设计参数，以优化压力与接近传感性能。

Method: 采用银包纱涂覆聚二甲基硅氧烷（PDMS），组装成一、二、四层加捻构型。系统研究了有效电极重叠面积和纤维间分离对电容响应的影响；基于局部单纤维接触面积计算应力（0.4-3.9 MPa）。通过改变层数调控机械强度、灵敏度、迟滞、热稳定性和循环耐久性，并构建4×4纺织传感阵列结合机械臂集成验证实时触摸与接近检测。

Result: 随着层数增加，断裂伸长率从37.5%提升至62.5%和85.0%，最大载荷从23.3 N增至42.7 N和89.7 N。四层传感器灵敏度在100 kHz下达0.1331 MPa⁻¹，具备低迟滞、25-90°C最小热漂移及15000次循环稳定性。接近检测范围随层数增加从60 mm降至40 mm，呈现架构依赖的灵敏度-检测范围权衡。4×4阵列实现空间接触映射，机器人系统端到端延迟为403 ms。

Conclusion: 纱线层级架构（层数和加捻构型）被确立为纺织集成电容传感系统的核心可调设计参数，直接影响压力灵敏度、机械性能、传感范围及人机交互响应。该研究为优化纺织传感器的架构设计提供了系统化的定量依据，推动其在可穿戴电子和机器人感知中的应用。
...
TL;DR: 本文提出一种控制信息感知的在线轨迹规划框架，通过运行时学习自身执行误差，动态调整空间约束并扩大自由规划区域，在保持时间最优的同时补偿累积执行误差。在自主赛车高保真闭环仿真中，该方法将圈时降低1.8秒且不增加计算负担（中位运行时间25ms）。


<details>
  <summary>Details</summary>
Motivation: 自主赛车在车辆动力学极限下运行，微小控制误差即引发安全风险和性能损失。传统轨迹规划器假设完美追踪而对执行误差视而不见，只能采用保守的空间裕度以保证安全，导致可用的赛道空间未被充分利用。

Method: 提出控制信息感知的在线轨迹规划框架：在运行时测量系统性跟踪偏差，动态调整空间赛道约束，迭代扩展自由空间规划区域；规划器保持时间最优，同时补偿累积执行误差。

Result: 在高保真闭环仿真环境中验证，圈时减少1.8秒，计算负担不增加，中位运行时间保持在25毫秒。
...
TL;DR: THRIVE是一个家庭康复平台，集成虚拟现实上肢康复游戏、基于摄像头的实时运动追踪系统和社会性互动机器人治疗师，专门针对上肢运动障碍儿童设计，通过机器人无关的模块化架构实现低成本、可扩展的家庭持续康复。


<details>
  <summary>Details</summary>
Motivation: 上肢运动障碍儿童需要通过持续、任务特异性的练习来改善功能，但传统康复训练缺乏趣味性且难以在家坚持，需要一种更具吸引力、负担得起且可适应性强的家庭康复方案，以提升治疗的连续性和依从性。

Method: 该平台采用三层架构：(1)一套新设计的虚拟现实康复游戏，涵盖弹出、击打、接住和抓取等任务，针对功能性伸手、抓握和物体操作动作进行训练；(2)基于摄像头的运动追踪系统实时捕捉儿童游戏过程中的运动学表现；(3)机器人治疗师（可部署为实体机器人教练或远程在场的虚拟代理），提供自适应动态反馈以激励儿童并引导其运动朝向治疗目标。该设计将治疗性游戏与机器人具体形态解耦，使系统支持多种机器人及不同人体形态。

Result: THRIVE提出了一个机器人无关（robot-agnostic）的模块化系统设计，能够将治疗性游戏与机器人具体形态分离，从而实现低成本、可扩展且易于适应家庭持续使用的康复平台，为儿童提供更一致和更引人入胜的上肢治疗路径。

Conclusion: THRIVE通过整合虚拟现实游戏、运动追踪和机器人治疗师，并采用解耦的模块化架构，为家庭环境中上肢运动障碍儿童提供了一种实用、可扩展且经济可行的康复方案，有望提高康复训练的持续性和治疗效果。
...
TL;DR: 本文提出将主动推断中的期望自由能（EFE）作为预算受限机器人信息路径规划的统一目标，通过高斯过程信念在硬路径长度约束下规划连续轨迹，同时实现精确地图构建与高价值区域定位，实验证明优于纯信息论基线。


<details>
  <summary>Details</summary>
Motivation: 自主机器人在未知环境探索（如火星找水）时面临双重需求：既要构建精确的信息地图，又要快速找到最高价值区域，同时还需为每一步移动和测量付出代价。经典的信息寻求准则和奖励寻求准则各自只解决其中一个目标，缺乏统一框架。

Method: 采用主动推断中的期望自由能（EFE）作为统一的动作选择目标，将机器人的信念建模为高斯过程，在硬路径长度约束下规划连续轨迹以最小化期望自由能，从而在单一框架下同时优化信息增益与奖励获取。

Result: 多次真实实验结果表明，基于EFE的规划方法能同时生成精确的后验地图并定位最高价值区域，在相同设置下优于基于信息论的基线方法。

Conclusion: EFE作为统一、易于调参的基于原则的信息采集策略，能够兼顾信息获取与价值搜寻两大目标，在机器人探索任务中促进自主部署并有效执行效率和资源约束。
...
TL;DR: 本文提出一个统一框架，通过"危险感知安全包络"将系统化危险分析与运行时强制执行桥接起来，采用跨层次安全转换过程（涵盖符号、空间和动态世界模型），并与物理AI运行时约束工具接口，以保障异构机器人在复杂城市环境中的安全移动。


<details>
  <summary>Details</summary>
Motivation: 异构机器人系统在多样化城市区域中部署时，在复杂的人机交互中维持安全性仍是一个关键挑战。传统方法将安全视为单个软件模块内的静态约束，缺乏系统性的跨层安全保障。

Method: 提出一个统一框架，包含：(1) 基于危险信息的"安全包络"（safety envelope），用于桥接系统化危险分析和运行时强制执行；(2) 跨层安全转换过程，跨越符号、空间和动态三类世界模型；(3) 与物理AI运行时约束工具（runtime harnesses）的接口集成，以保障城市移动安全。

Result: 该框架能够将危险分析结果转化为可在运行时执行的安全约束，并通过与物理AI运行时约束工具的自然对接，为城市环境中的安全移动提供形式化保证。

Conclusion: 该统一框架展示了如何通过跨层安全转换过程，将系统化危险分析有效地转化为运行时可执行的安全保证，为异构机器人系统在城市环境中的安全部署提供了新的方法论路径。
...
### [27] [Refined outer-approximation algorithms for monotonic optimisation](https://arxiv.org/abs/2608.13694)
*Ahmed Rashwan*

Main category: math.OC

TL;DR: 本文针对单调优化中的polyblock外逼近算法（POA）扩展性差的问题，提出了三项算法改进：广义锚点选择（最优平衡单调性切割）、松弛最优性条件（无需连续性假设的有限终止保证）和向量化并行变体，并设计了高效的树形数据结构实现，显著加速POA核心子程序。数值实验表明改进方法比标准POA提速一个数量级，能解决现有变体失败的难题。作者还开源了Python包polyblocks提供一个开发新POA变体的框架。


<details>
  <summary>Details</summary>
Motivation: 单调优化是一类广泛的非凸问题，常用POA算法求解。但POA存在严重的扩展性问题：描述外部逼近所需的顶点数可能呈指数增长，导致内存占用大、子程序越来越昂贵，限制了其在实际问题中的应用。

Method: 提出三项核心改进：1) 广义锚点选择，生成最优平衡单调性切割；2) 松弛最优性条件，在不需连续性假设下保证有限终止；3) 向量化变体，并发处理多个节点。此外开发了基于树形结构的高效实现，加速POA核心子程序并紧凑存储外部逼近。

Result: 数值实验表明所提出的改进相比标准POA实现了数量级的加速，并能成功求解现有变体无法处理的问题。

Conclusion: 通过算法改进和高效数据结构实现，显著提升了POA的效率和可扩展性，同时发布了开源的polyblocks包以促进该领域算法开发与复现。
...
### [28] [Multi-period Value-at-Risk Constrained Portfolio Optimization via DC Programming](https://arxiv.org/abs/2608.13748)
*Thi Thu Van Nguyen*

Main category: math.OC

TL;DR: 本文提出一种投影惯性增强差分凸函数算法（iBDCA），用于求解带有限场景VaR约束、交易成本和分散化正则化的多期组合优化问题，并从理论和数值两方面验证了算法的有效性。


<details>
  <summary>Details</summary>
Motivation: 多期组合优化中同时纳入VaR约束、交易成本和分散化正则化会导致非光滑、非凸的优化问题，现有DCA和BDCA等算法在收敛速度和求解质量上存在不足，需要设计更高效的算法。

Method: 利用有限场景VaR-CVaR恒等式，将原问题推导为凸组合集上的惩罚差分凸（DC）规划形式；提出投影惯性增强BDCA算法（iBDCA），结合惯性外推、目标值保护机制和增强线搜索；在局部无平局条件下证明全序列收敛及局部R-线性收敛率。

Result: 理论证明算法良定义、目标序列单调递减且聚点均为临界点；在局部无平局条件下全序列收敛并具有局部R-线性收敛率。数值实验表明iBDCA优于DCA和标准BDCA，样本外回测对比等权与买入持有基准揭示收益、风险、交易成本与VaR控制之间的权衡。

Conclusion: iBDCA算法为求解带VaR约束的多期组合优化问题提供了高效且理论上可收敛的求解框架，数值实验验证了其在实际应用中的有效性和竞争性。
...
### [29] [Best Response Dynamics for Zero-Sum Dynamic Games with Partial-Asymmetric Information](https://arxiv.org/abs/2608.13755)
*Yuxiang Guan,Iman Shames,Tyler H. Summers*

Main category: math.OC

TL;DR: 本文研究部分且非对称信息下的一类零和随机线性二次动态博弈（LQDGs），提出基于最佳响应动态的方法来解决信息不对称带来的信念表示和心智理论挑战，推导出纯线性动态输出反馈控制策略下每个玩家的最佳响应显式表达式，并通过数值实验揭示高阶信念状态虽导致无限维内部状态，但游戏价值在几次迭代后即收敛，表明高阶信念收益递减。


<details>
  <summary>Details</summary>
Motivation: 信息不对称给零和随机线性二次动态博弈带来了信念表示和心智理论的根本性挑战，玩家必须推断其他玩家的信念状态和估计来制定策略。现有研究表明动态规划式的分解方法难以应用于此类问题，因此需要探索替代方法。

Method: 提出基于最佳响应动态的替代方法。推导了纯线性动态输出反馈控制策略类别内每个玩家的最佳响应显式表达式，其中每个控制器的内部状态维度是系统状态维度的整数倍。玩家迭代更新最佳响应会形成越来越高阶的信念状态，导致内部状态无限维化。通过数值实验分析非对称信念、信念阶数、相对可控性和可观测性以及直接馈通对博弈价值的影响。

Result: 数值结果表明博弈价值在仅仅几次迭代后就收敛，说明高阶信念状态带来的收益逐渐消失。此外，数值实验揭示了非对称信念、信念阶数、相对可控性和可观测性及直接馈通对线性二次追逃博弈价值的影响。

Conclusion: 最佳响应动态为部分非对称信息下的LQDGs提供了一种可行的求解框架，能够有效应对信念表示和心智理论挑战。尽管理论上需要无限维内部状态，但实际中低阶信念即可逼近博弈价值，具有重要的计算优势。
...
### [30] [Recursive Filtering and Stochastic Control under Finite Partition-Based Observations](https://arxiv.org/abs/2608.13825)
*Saul Díaz-Infante Velasco,Yofre H. Garcia,Jesús Adolfo Minjárez-Sosa*

Main category: math.OC

TL;DR: 本文为部分可观测随机系统提出了一个基于有限可测划分观测的滤波与最优控制框架，覆盖了阈值、量化、审查、事件触发和间歇观测等区域信息机制，并将原始部分可观测问题转化为完全可观测的马尔可夫决策过程，同时提出了类相关的有限维近似方法及误差界。


<details>
  <summary>Details</summary>
Motivation: 部分可观测随机控制系统中的观测往往不是完全信息，而是仅能识别隐藏状态所属的某个有限可测划分类（如阈值、量化、事件触发等区域信息机制）。现有方法缺乏一个统一的数学框架来同时处理这些结构，且可观测类中可能包含原子、连续或混合成分，导致滤波和最优控制问题在度量空间上具有无穷维复杂性。

Method: 首先在测度层面构造：对每个可观测类定义类受限的非归一化条件测度，通过其预测概率归一化得到后验分布。基于该递推关系构建信息状态（由已观测类和支撑在其上的条件测度组成），将原问题转化为完全可观测的马尔可夫决策过程。推导折扣成本准则下的贝尔曼方程，建立平稳最优策略存在性条件。针对信息空间的无穷维特性，提出类相关的有限维逼近方法以同时保留连续部分和原子质量，并推导近似滤波误差与值函数误差之间的抽象界限。

Result: 建立了统一的滤波-最优控制框架，提出了类相关有限维近似方案，得到了逼近误差到值函数误差的传递界限，并通过基于直方图近似的参考模型验证了该构造的有效性。

Conclusion: 该框架为覆盖多种区域信息观测机制的部分可观测随机系统提供了统一的滤波与最优控制方法论，其有限维近似方法在保留混合分布结构的同时保证了最优策略的可计算性。
...
### [31] [Nondegeneracy Conditions for Control Problems with Nonregular Mixed Constraints](https://arxiv.org/abs/2608.13834)
*Jorge Becerril,Karla Cortez*

Main category: math.OC

TL;DR: 本文研究带单个非正则混合不等式约束的最优控制问题中，乘子为纯有限可加集函数（charge）时的非退化性与正态性条件。证明了在模型问题非正则瞬间退化是内在且不可避免的，并提出四个可验证条件逐步保证非退化乘子、纯charge消失、正态性及charge质量显式界。


<details>
  <summary>Details</summary>
Motivation: 在非正则混合约束下，传统的最优控制必要条件的乘子可能退化为纯charge，而自然正则逼近的归一化乘子不存在L∞意义下的弱*极限。这种退化现象使经典的必要条件失去有效性，需要系统性地理解退化机制并提出可验证的非退化条件。

Method: 首先对单个时刻非正则的模型问题证明退化是本质且不可避免的，即每个容许过程都通过纯charge满足退化性必要条件，且正则逼近的归一化乘子在Banach极限意义下仅产生纯charge。随后提出四个逐级增强的可验证条件，分别保证非退化乘子、纯charge消失、正态性以及charge质量的显式上界。此外，在非正则末端时刻建立了将charge质量与成本乘子联系起来的平衡恒等式。

Result: 证明了模型问题中退化性无法避免；提出了四个渐进增强的可验证条件可分别实现非退化性、纯charge消除、正态性和charge界控；并推导出末端时刻的平衡恒等式，用于判定完全异常性与正态性。提供了实例验证各条件的可实现性并讨论其局限性。

Conclusion: 本文完整刻画了非正则混合约束下最优控制问题中纯charge乘子的出现机制，给出了从退化到正态的逐步充分条件，为处理非正则约束下的最优控制问题提供了理论工具和可验证准则。
...
### [32] [First-Order Optimization as Minimum-Time Control](https://arxiv.org/abs/2608.13915)
*Liraz Mudrik,Isaac Kaminer,Pramod P. Khargonekar*

Main category: math.OC

TL;DR: 该论文将一阶优化重新表述为最小时间控制问题，把迭代点视为状态、梯度组合视为控制、梯度范数阈值内的点视为目标集，从而将实例复杂度定义为值函数（最小查询次数），而非传统的最坏情况复杂度。在强凸二次型上，共轭梯度迭代自然从离散Pontryagin条件中涌现，值函数成为可控性指数；超越二次型后，Hessian生成的可达跨度替代了可控性矩阵，可行性变为可达性问题。论文还证明曲率是一种资源：存在可检验条件保证某些实例所需的步数少于其极小点处二次模型的可控性指数，且该差距可随维度无界增长。


<details>
  <summary>Details</summary>
Motivation: 传统一阶优化复杂度分析主要关注类上的最坏情况（worst-case）复杂度，但这种度量无法刻画单个实例的真实难度，导致算法评估与实例具体特征脱节。作者希望通过控制论视角将复杂度定义在实例层面，以更精确地揭示问题内在难度，并为任意一阶方法提供可比的基准。

Method: 将迭代框架建模为最小时间控制问题：状态为迭代点，控制为迄今观测到的梯度的组合更新，目标集为梯度范数不超过给定容差的点集。对固定目标函数和起点，到达目标所需的最小Oracle查询次数定义为值函数。在强凸二次形上利用离散Pontryagin极大值原理导出共轭梯度迭代，并以可控性指数解释值函数；对非二次情形，引入Hessian生成的可达跨度替代可控性矩阵，通过可达性判定临界点是否可达及可达步数。此外提出一个可检验条件，用于判定实例是否比其极小点处二次模型的可控性指数需要更少步数。

Result: 在强凸二次型上，共轭梯度迭代由离散Pontryagin条件自然涌现，值函数对应可控性指数。对于非二次目标，通过Hessian生成的可达跨度判断可行性（即临界点可达性与步数），并证明曲率是加速资源：存在可检验条件表明某些实例所需步数可少于对应二次模型的可控性指数，且该加速差距随维度增长可无界扩大。

Conclusion: 值函数为单个实例的内在难度提供了精确基准，使任何一阶方法都能被直接度量，超越了传统最坏情况分析框架。曲率被证明是一种可利用的资源，其对复杂度的改善可以随维度无界放大，为设计实例自适应的一阶优化算法提供了理论依据。
...
### [33] [Benders Decomposition with Partial Non-Anticipativity Relaxation for Multi-Stage Stochastic Clean Energy Transition Planning](https://arxiv.org/abs/2608.14105)
*Ahmet Emir Şener,Burak Kocuk,Tuğçe Yüksel*

Main category: math.OC

TL;DR: 本文研究了校园级电-热综合能源系统在战略层和运营层双重不确定性下的清洁能源转型规划问题，提出了多阶段随机混合整数规划模型与带部分非预期性松弛的Benders分解算法，并通过中东技术大学校园案例验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 校园级综合电-热系统的清洁能源转型规划需要同时考虑战略层面的长期不确定性（技术成本和效率的随机演化）和运营层面的短期不确定性（需求、可再生能源出力、热传递性能的波动），现有方法难以在合理计算预算内求解如此大规模的组合优化问题。

Method: 构建多阶段随机混合整数规划模型联合优化投资与运营决策；基于箱式不确定性集合推导鲁棒重构以应对短期运营不确定性；开发部分非预期性松弛的Benders分解算法——投资变量及其非预期性约束保留在主问题中，运营变量分配给场景子问题并松弛其非预期性约束，终止时通过一个小规模线性规划恢复运营变量的非预期性，并证明该修正步骤只会使目标函数值增加有限界；通过有效不等式和两阶段割添加策略提升计算性能。

Result: 在中东技术大学校园案例中，所提方法能在合理计算预算内以高时间分辨率求解模型，而扩展形式或经典Benders分解无法做到。滚动时域蒙特卡洛模拟和样本外分析表明，增加的鲁棒性可显著提升转型计划的运营可靠性，同时总成本仅适度增加。

Conclusion: 提出的鲁棒多阶段随机优化框架和带部分非预期性松弛的Benders分解算法为校园级综合能源系统转型规划提供了一种高效且可靠的求解途径，在可接受的成本代价下实现了显著的可靠性提升。
...
### [34] [Joint Inventory Placement, Assortment Personalization, and Order Fulfillment for Substitutable Products](https://arxiv.org/abs/2608.14275)
*Mikhail Fadin,Omar El Housni,Huseyin Topaloglu*

Main category: math.OC

TL;DR: 本文研究在线零售商在分布式仓储网络中的联合优化问题，同时决策库存分配、个性化商品推荐和订单履约，提出了具有理论保证的近似算法，在多项式罗吉特（MNL）选择模型下获得常数因子近似比。


<details>
  <summary>Details</summary>
Motivation: 现代在线零售商依赖分布式仓储网络快速履约订单。然而，库存分配、个性化商品推荐和订单履约这三个关键决策相互耦合，此前缺乏统一的模型和高效求解算法。本文旨在提出一个联合优化框架，并设计可证明高效的算法来解决这一复杂问题。

Method: 构建一个联合决策模型：销售期初确定各仓库的库存配置（受容量和供应约束），随后在有限时间范围内按已知分布离散到达不同类型顾客，顾客根据离散选择模型从推荐集合中至多选择一种商品，企业再从可行仓库中选择履约仓库以获取利润。针对一般概率选择模型，提出随仓库容量和产品供应规模扩展的渐近最优近似算法；针对MNL选择模型，提出常数因子近似算法。

Result: 在一般概率选择模型下，建立了渐近最优的近似算法。在MNL选择模型下，获得常数因子近似比，最一般设定下为0.080，在平稳性和产品供应无限的设定下（仅存在仓库容量约束）达到0.199。这是首次为库存放置、个性化推荐与订单履约联合问题提供可证明高效算法。

Conclusion: 本文为分布式仓储网络下的库存分配、个性化推荐和订单履约联合优化问题提供了首个具有理论保证的高效算法框架，在一般选择模型和MNL模型下分别实现渐近最优和常数因子近似，兼具理论可行性与强实证表现。
...
### [35] [Diffusion-Based Policies for Dynamic Control of Stochastic Processing Networks](https://arxiv.org/abs/2608.14289)
*Baris Ata,Jin Guang,J. Michael Harrison,Nian Si*

Main category: math.OC

TL;DR: 本文提出了一种通用的多类排队网络布朗控制问题（BCP）求解方法，允许同一类工作有多种处理方式、可拒绝输入流，通过数值求解高维BCP并将其转化为可实施的调度策略，在重流量参数区域内近似最优。


<details>
  <summary>Details</summary>
Motivation: 实际排队网络通常有多类工作、多种处理方式以及可拒绝的输入流，但现有的近似控制方法要么计算效率低、要么难以在原网络中解释。需要一种既能在高维（m≥50）下高效求解，又能产生可理解可实施的策略的方法。

Method: 将原始排队网络建模为布朗控制问题（BCP），其状态空间为m维非负象限，控制为从有界多面体集中选择的漂移向量，基于动态状态观测。利用近期开发的数值计算方法求解BCP（维度可达m=50），并将数值解转化为原排队系统中可实施的控制策略。

Result: 数值例子支持该策略在重流量参数区域内近似最优的猜想；与之前基于低维"等效工作负载公式"的方法相比，该方法虽计算成本更高但更易于在原网络中解释和实施。

Conclusion: 所提出的BCP数值求解及策略转化方法为多类、多重处理方式、可拒绝输入流的排队网络提供了一条可行的近似最优控制途径，兼具高维可计算性和策略可解释性。
...
### [36] [Optimal Control with $L^\infty$ and Integral Cost Functionals](https://arxiv.org/abs/2608.14316)
*Madhu Dhiman,Veeraruna Kavitha,Nandyala Hemachandra*

Main category: math.OC

TL;DR: 本文研究同时包含累积成本（积分成本）和峰值成本（上确界/L∞成本）的混合最优控制问题，建立最优解的存在性并通过光滑逼近方法构造ε-最优纯控制解，最后在排队问题中验证了该方法可在峰值拥堵与累积性能之间实现权衡。


<details>
  <summary>Details</summary>
Motivation: 经典控制问题通常分别处理累积性能（积分成本）或峰值/最坏情况性能（上确界成本），但实际系统中两者往往同样重要。虽然每种准则已被充分研究，但二者的组合控制问题尚未被广泛探索，本文正是针对这类组合目标的控制问题开展研究。

Method: 首先在松弛控制框架下利用概率分布的控制空间，借助其紧致性和凸性建立最优松弛控制的存在性，进而证明对任意ε>0存在ε-最优纯（经典）控制。针对由上确界项引起的非光滑性导致动态规划原理不成立的问题，引入一族光滑逼近将原问题转化为标准控制问题，并利用最大性定理（Maximum Theorem）证明光滑问题的最优解构成原问题的ε-最优解。

Result: 建立了组合成本控制问题的最优解存在性；证明了光滑逼近问题的解对原问题构成ε-最优解，且当光滑参数趋于零时ε趋于零；在排队问题中验证了所提方法能够实现峰值拥堵水平与累积性能之间的权衡。

Conclusion: 本文为同时考虑累积与峰值性能的组合成本控制问题提供了系统的存在性理论和可计算的近似求解方法，并通过排队问题展示了其实际应用价值。
...
### [37] [Optimizing bounds for energy-constrained optimal cooling problems in two dimensions](https://arxiv.org/abs/2608.14334)
*Pedro Blöss Braga,Giovanni Fantuzzi*

Main category: math.OC

TL;DR: 本文研究二维不可压缩流体最优冷却控制问题，通过拉格朗日对偶建立良定的对偶问题，用半定规划层次法数值求解，并给出新的解析上界：任意域中冷却效率不超过 Pe²，而在圆盘/环域中可改进为 Pe²/ln²Pe。


<details>
  <summary>Details</summary>
Motivation: 为能量受限的冷却流动问题（冷边界+内部热源/热汇）提供新的冷却效率上界，并推广和改进了现有关于圆盘中冷却流动的已知效率界。

Method: 利用拉格朗日对偶构造良定的对偶问题以获得冷却效率上界；通过离散化构造收敛的半定规划（SDP）层次法进行数值逼近，并利用问题对称性降低计算复杂度；同时构造对偶问题的可行点以获得新的解析上界。

Result: 在任意域和热分布下证明了 E(Pe) ≲ Pe²，且在圆盘和环域中当热源/热汇分布具有正方位角平均时，改进为 E(Pe) ≲ Pe²/ln²Pe。数值实验在方形和环形域上验证了方法的有效性。

Conclusion: 所提出的对偶+SDP数值框架能有效计算最优冷却效率上界，新解析界推广并改进了已有圆盘结果，对一般域提供了统一的 Pe² 界。
...
### [38] [A Shrinkage Path Heuristic for Wasserstein Distributionally Robust Optimization](https://arxiv.org/abs/2608.14336)
*Lingjun Meng,Ryan Cory-Wright,Wolfram Wiesemann*

Main category: math.OC

TL;DR: 本文提出一种收缩路径启发式方法，将Wasserstein分布鲁棒优化（DRO）问题简化为在SAA解与经典鲁棒优化解之间的线段上的一维搜索，在显著降低计算成本的同时，能在多商品报童和预约调度问题中实现Wasserstein DRO相较于SAA的大部分样本外性能提升。


<details>
  <summary>Details</summary>
Motivation: Wasserstein分布鲁棒优化（DRO）是处理不确定决策问题的常用框架，但其标准确定性重构中包含非凸的内层子问题，求解困难。现有方法计算代价高昂，亟需一种既能降低计算复杂度又能保持较好性能的求解策略。

Method: 提出收缩路径（shrinkage path）启发式算法，将DRO问题转化为连接样本均值近似（SAA）解与经典鲁棒优化解之间线段上的一维搜索。在特定设定下推导先验次优性界，在一般情形下通过对偶公式应用类似启发式获得后验界。

Result: 在多商品报童问题和预约调度问题的数值实验中，收缩路径启发式能以极小的计算成本获得Wasserstein DRO相对于SAA样本外性能改进的85%-110%（报童）和45%-70%（预约调度）。

Conclusion: 收缩路径启发式为求解Wasserstein DRO提供了一种计算高效且实用的近似途径，能够在保持大部分性能收益的同时大幅降低求解难度，并在不同问题类型中表现出可靠的性能水平。
...
### [39] [AI-Assisted Discovery and Construction of a Counterexample to the Convergence of Three-Block ADMM with the Identity Matrix as its Third Constraint Block](https://arxiv.org/abs/2608.14396)
*Kenan Xu,Xiangfeng Wang*

Main category: math.OC

TL;DR: 本文证明直接三块ADMM即使在前两块为强凸二次函数时也可能不收敛，构造了周期为66的有界非收敛轨道的显式有理反例，并研究了乘子松弛恢复收敛的条件。


<details>
  <summary>Details</summary>
Motivation: 三块ADMM的直接推广可能不收敛已有反例，但当第三块为恒等约束时该问题悬而未决——文献中既无一般收敛性证明也无反例。

Method: 利用Codex与GPT-5.6 Sol构建显式有理反例，沿分段仿射降维路径进行验证；通过乘子松弛分析恢复收敛的条件（问题依赖的小步长可行、均匀正步长不可行）；另用Kimi Code与Kimi K3模型独立获得周期23的局部吸引子证书。

Result: 给出三块ADMM在第三块为恒等约束时不收敛的显式反例（周期66的有界非收敛轨道），并证明：针对具体实例存在小步长乘子松弛可恢复收敛，但不存在对全类统一有效的正相对步长。

Conclusion: 直接三块ADMM在第三约束块为恒等时依然可能失败，即使前两块为强凸二次；不同研究工具链会影响所发现的数学对象与证书类型。
...
### [40] [Residual-Controlled Douglas--Rachford Splitting for Differentiable Solver Layers](https://arxiv.org/abs/2608.14470)
*Kang Liu,Jianchen Hu*

Main category: math.OC

TL;DR: 提出残差控制Douglas-Rachford分裂（RCDRS），一种用于锥线性规划的可微求解器层。它将展开求解器视为反馈控制动态系统，通过因果控制器自适应调节松弛与目标驱动参数，在保持投影分裂结构的同时，理论证明固定块仍为平均松弛DRS算子且具有有限步定点残差界，实验表明其优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 固定的展开深度必须在解质量、可行性和计算预算之间进行权衡，缺乏自适应的求解策略。现有可微求解器层难以同时兼顾收敛精度与端到端学习效率，需要一种能够自适应调整求解路径的方法。

Method: 将展开的求解器建模为反馈控制的动力系统：因果控制器根据当前残差状态动态调整松弛参数和目标驱动参数，同时保持Douglas-Rachford分裂的投影拆分结构。理论分析包括：固定可容许块的DRS算子性质、时变rollout作为极限平均算子的可加扰动分析、从最终分裂状态恢复原始-对偶终止诊断。

Result: 在混合锥基准测试和工程应用上，RCDRS在解质量、可行性以及下游决策性能方面均优于基线方法，同时保持了可微求解器层的端到端训练能力。

Conclusion: RCDRS通过将可微求解器重新解释为受控动力系统，有效解决了固定深度展开的精度-可行性-预算折中问题，为可微优化层提供了理论保障与实用性能的统一框架。
...
### [41] [On quantitative sufficient second-order optimality conditions for elliptic optimal control problems](https://arxiv.org/abs/2608.14525)
*Francisco Fuica,Nicolai Jork*

Main category: math.OC

TL;DR: 本文针对无Tikhonov正则化的半线性椭圆方程盒约束分布式最优控制问题，提出了一种保证线性化状态L²范数下强制性的定量二阶最优性条件，并证明了该条件在状态和跟踪数据扰动下的稳定性。


<details>
  <summary>Details</summary>
Motivation: 无Tikhonov正则化的最优控制问题中，二阶变分是线性化状态的二次型，其曲率系数函数可能消失或变号，导致经典二阶充分条件失效。需要一个定量的条件来保证强制性和二阶条件的稳定性。

Method: 研究了无Tikhonov正则化的半线性椭圆方程盒约束分布式最优控制问题，提出了一个定量条件来确保关于线性化状态L²范数的强制性（coercivity），进而推导二阶条件的稳定性结论。

Result: 给出了一个定量条件，该条件蕴含关于线性化状态L²范数的强制性，并由此证明了二阶条件在状态扰动和跟踪数据扰动下的稳定性。

Conclusion: 在一个曲率系数可能消失或变号的非标准设定下，通过引入定量条件恢复了二阶最优性条件的有效性及其在数据扰动下的稳定性。
...
### [42] [Consistent Model Chasing Is Minimax Optimal: The Exact Value of Scalar Adversarial Adaptive Control under Large Parametric Uncertainty](https://arxiv.org/abs/2608.13651)
*Dimitar Ho*

Main category: eess.SY

TL;DR: 本文精确求解了一个对抗性扰动下自适应控制的基本问题：标量系统 x_{t+1}=ax_t+u_t+w_t，其中未知极点 a∈[-Δ,Δ]，Δ任意大。证明了最坏情况峰值 ||x||_∞ 的博弈值（最优控制器可保证的性能）为 γ^*(Δ)=1+Δ，其中 1 是扰动带来的不可约代价，Δ 是一次不可避免的识别尖峰的确切代价。最优策略是中点处的确定等价死区控制，属于鲁棒预言机×一致模型追逐架构，且该架构是被迫的而非仅充分。这是对一致模型追逐作为对抗自适应控制设计原则的首个精确最优性认证。


<details>
  <summary>Details</summary>
Motivation: 尽管标量线性系统在自适应控制中看似基础，但从未有人确定在参数不确定性任意大小、对抗性扰动下，最坏情况峰值响应的精确博弈值。现有理论仅提供稳定性证明、增益界和遗憾率，而非精确的性能值。作者旨在填补这一空白，回答"最优的因果控制器能够保证的最小最坏情况峰值是什么"。

Method: 作者将问题形式化为一个二人零和博弈，其中对抗者选择未知极点 a 和扰动序列 w。关键方法包括：(1) 通过定义 θ_t := -u_t/x_t，将任意因果控制器分解为"预言机-选择器"组合，证明该架构是必然的而非仅充分的；(2) 在临界历史处用最优性条件将选择器钉在中点上；(3) 对经典工具（探测、承诺、乐观、遗憾）逐一进行量化失败分析；(4) 最终建立值函数 γ^*(Δ)=1+Δ，最优策略为集合成员一致性区间中点的确定性等价死区控制。

Result: 证明了博弈值 γ^*(Δ)=1+Δ 对所有 Δ>0 成立。最坏情况峰值由两部分组成：扰动带来的不可约代价 1 和由单次不可避免的识别尖峰产生的代价 Δ。最优策略是中点处的确定等价死区控制（鲁棒预言机×一致模型追逐架构），它不含探索机制，学习纯粹是被动的。同时量化了经典工具的失败：探测在回报之前就受到惩罚，承诺在次扰动激励下是致命的，乐观方法退化为平局决定或渐近付出至少两倍最优代价，遗憾证书对最坏情况峰值双向失明。

Conclusion: 这一结果首次为一致模型追逐作为对抗自适应控制的设计原则提供了精确的最优性证书。它表明在该问题设定下，被动学习配合集合成员一致性区间中点控制不仅简单，而且严格最优——任何主动探索或复杂策略都无法改善最坏情况峰值性能。该工作为自适应控制的精确性能理论开辟了新的方向。
...
### [43] [Control Barrier--Value Functions under Partial Observability: Safety Guarantees via Conformal Prediction](https://arxiv.org/abs/2608.13819)
*Niloofar Jahanshahi,Mo Chen*

Main category: eess.SY

TL;DR: 本文针对部分可观测非线性控制系统，将控制障碍-值函数（CBVF）框架扩展到基于估计状态的场景。利用共形预测获得估计误差界，在估计空间中建立安全证书，推导有限时域概率安全保证，并提出基于QP的在线安全滤波器。在部分可观测避障案例中验证。


<details>
  <summary>Details</summary>
Motivation: 现有CBVF框架结合了Hamilton-Jacobi可达性和控制障碍函数，但假设全状态可观测。现实系统中往往无法获得完整状态信息，控制器需基于估计状态工作，这给安全性分析与综合带来了新的挑战。

Method: 1) 给定估计器，利用共形预测（conformal prediction）对估计误差进行量化，在用户指定的误覆盖率水平下获得误差界；2) 将该误差界融入估计空间的安全分析，定义面向部分可观测系统的CBVF安全证书；3) 推导真实系统状态的有限时域概率安全保证；4) 针对控制和扰动仿射的系统，提出基于二次规划（QP）的在线安全滤波器，实时强制CBVF安全条件以抵御有界扰动。

Result: 通过部分可观测的避障案例研究验证了所提出框架的有效性。

Conclusion: 该工作将CBVF安全框架从全状态可观测扩展到部分可观测场景，利用共形预测为估计不确定性提供可调的统计保证，并实现了实时在线安全滤波，为基于估计器的不确定系统中安全控制提供了概率性保障方法。
...
### [44] [Hierarchical Sensor-Spoofing Defence Framework for Networked DC Microgrids via Cyber-Physical Coordination](https://arxiv.org/abs/2608.13823)
*Mengxiang Liu,Xin Zhang,Shiyi Zhao,Ruilong Deng*

Main category: eess.SY

TL;DR: 本文提出一种分层框架，通过多层网络-物理协调来检测和缓解网络化微电网（NMGs）中的传感器欺骗攻击。该框架仅在微电网的公共耦合点（PCC）部署物理防护技术，基于安全的传感器读数采用网络检测/缓解算法，并利用MG-DER协调的主动检测方案触发参数扰动以揭露智能传感器欺骗攻击，随后通过协调缓解方案递归估计传感器偏差。


<details>
  <summary>Details</summary>
Motivation: 现有针对传感器欺骗攻击的防御方法（物理防护和网络检测/缓解）在单个DER中各有优缺点，难以在成本、安全和性能之间取得平衡，且缺乏跨DER的多层高效协调的系统性研究。

Method: 提出一个分层框架，仅在MG的PCC关键点部署物理防护技术，基于安全的传感器读数支撑网络检测/缓解算法；采用MG-DER协调的主动检测方案，通过策略性触发参数扰动揭露智能传感器欺骗攻击；随后激活基于MG-DER协调的缓解方案，递归且精确地估计传感器偏差。

Result: 在物理-网络DC NMG测试平台上进行了实验，验证了该框架在多种攻击场景下的有效性。

Conclusion: 该分层网络-物理协调框架能够以较低成本部署要求实现跨DER的传感器欺骗攻击的有效检测与缓解，为网络化微电网安全提供了系统性解决方案。
...
### [45] [Real-Time In-Domain Congestion Control for the LWR Traffic Model via Control Barrier Functions](https://arxiv.org/abs/2608.13841)
*Zehang Zhu,Brian Block,Stephanie Stockar*

Main category: eess.SY

TL;DR: 本文提出基于控制障碍函数（CBF）的实时域内拥塞控制方法，用于Lighthill-Whitham-Richards交通模型，将拥塞控制转化为分布式安全控制问题，通过离散化和KKT分析转化为高维二次规划，并开发了结构利用的主对偶有效集算法实现实时求解，数值仿真验证了方法的有效性和实时可行性。


<details>
  <summary>Details</summary>
Motivation: 交通拥塞是实际交通系统中的关键问题，需要实时有效的控制方法。Lighthill-Whitham-Richards模型作为经典的宏观交通流模型，其拥塞控制问题涉及无限维优化，难以直接实时求解，因此需要开发高效的算法来实现实时安全控制。

Method: 首先将交通拥塞表述为分布式安全控制问题，形成无限维优化问题；然后通过空间离散化和KKT（Karush-Kuhn-Tucker）条件分析，将无限维问题转化为高维二次规划（QP）；最后开发一种利用问题结构的主对偶有效集（primal-dual active set）算法，在保证收敛性的前提下实时计算安全控制输入。

Result: 通过使用不同名义控制器的数值仿真，验证了所提方法在交通拥塞控制方面的有效性，并证明了其具备实时求解的可行性。

Conclusion: 该研究成功将控制障碍函数方法推广到LWR交通模型的拥塞控制中，开发的结构化主对偶有效集算法能够在不牺牲实时性的前提下提供安全控制输入，为交通拥塞的实时控制提供了一种理论上有保证、计算上可行的解决方案。
...
### [46] [Feedback-Guided DNN-Based Controller Fusion for Robust Fixed-Parameter Active Noise Control](https://arxiv.org/abs/2608.14061)
*Lu Bai,Yiming He,Xiaofeng Nan,Kai Chen,Jing Lu*

Main category: eess.SY

TL;DR: 本文提出了一种基于反馈引导的DNN控制器融合框架，用于鲁棒的固定参数主动噪声控制（ANC）。该方法结合因果关系WaveNet控制器与反馈引导的混合专家（MoE）模块，通过门控网络根据当前声学条件估计多个预训练FIR专家的权重，在无需在线参数更新的情况下提升对不同声学条件的鲁棒性。模型完全因果且支持逐样本流式推理，计算开销均匀分布在采样点上以降低峰值计算负载。耳机ANC实验结果表明，该方法在低频段实现了显著的降噪效果，同时在1-8 kHz频段噪声放大可忽略不计。


<details>
  <summary>Details</summary>
Motivation: 自适应ANC方法可能遭受不稳定性或发散问题，限制了实际部署；而固定参数控制器在噪声特性和声学路径条件变化时性能显著下降。因此需要一种既保持固定参数部署的稳定性，又能适应变化声学条件的鲁棒ANC方案。

Method: 提出反馈引导的DNN控制器融合框架，包含两部分：(1) 因果关系WaveNet控制器作为主干；(2) 反馈引导的混合专家（MoE）模块，其中门控网络根据当前声学条件（通过反馈信号估计）动态分配多个预训练FIR专家的权重。模型完全因果，支持逐样本流式推理，并通过将计算量均匀分布到各采样点来降低峰值计算负载。

Result: 在耳机ANC实验上，该方法在低频段实现了显著的降噪效果，同时在1-8 kHz范围内几乎没有噪声放大，验证了在变化声学条件下无需在线参数更新即可保持鲁棒性能的有效性。

Conclusion: 反馈引导的MoE融合机制有效解决了固定参数ANC控制器在声学条件变化时的性能退化问题，在保持计算高效和流式推理能力的同时实现了鲁棒降噪，为ANC系统的实用化部署提供了一种有前景的方案。
...
### [47] [A Temporal Barrier Framework for Collision Avoidance in Multi-Agent Autonomous Aerial Vehicles](https://arxiv.org/abs/2608.14239)
*Benedikt Barthel Sorensen,Mitchell Black,Erfaun Noorani,Themistoklis Sapsis*

Main category: eess.SY

TL;DR: 提出对抗性碰撞时间（aTTC）风险度量，并将其嵌入控制障碍函数（CBF）框架中，形成以时间而非距离或速度定义的aTTC-CBF，使自主飞行器编队在动态、不确定、对抗性环境中既能安全飞行又能高效完成任务。实验表明其航点进度提升两倍、碰撞率降低一半。


<details>
  <summary>Details</summary>
Motivation: 在动态、不确定且可能对抗的环境中运营自主飞行器编队，需要既可靠又具选择性的安全协议，使代理能在近距离飞行同时推进任务目标。传统的基于距离或速度的CBF安全屏障往往过于保守，限制了任务进度。

Method: 提出aTTC风险度量，量化给定代理被任意周围代理在假设对抗意图下最快被触及的时间；将其嵌入CBF框架，直接在时间域定义屏障而非距离或速度域；利用可微神经网络代理使aTTC可在标准CBF二次规划中实时计算。

Result: 在3D独立追逐和编队飞行场景的长时域仿真中，aTTC-CBF相比基于高阶距离的CBF基线，实现了最多两倍的航点进度，同时碰撞率降低一半。

Conclusion: aTTC-CBF具有固有的预见性，代理根据对手在其动力学约束下能够多快到达碰撞来调节自身速度，而非仅判断是否处于碰撞航线上，从而在安全性与任务效率之间实现更优平衡。
...
### [48] [Diagonalizable Directed Laplacians by Positive Arc-Weight Design for Master Stability Analysis](https://arxiv.org/abs/2608.14439)
*Aandrew Baggio Sahaya Arokiadoss*

Main category: eess.SY

TL;DR: 本文证明每个弱连通有向图都存在一个严格正弧加权，使其加权入度拉普拉斯矩阵可对角化，从而恢复主稳定函数（MSF）变分方程的完全解耦形式。


<details>
  <summary>Details</summary>
Motivation: 传统MSF公式依赖网络拉普拉斯矩阵可对角化，从而将变分方程分解为独立方程。但有向图的拉普拉斯矩阵不一定可对角化，这限制了MSF方法在有向网络中的应用。作者旨在通过构造合理的弧加权方案，使得有向图的拉普拉斯矩阵可对角化。

Method: 提出一个两步构造方法：首先从弱连通有向图中提取一个弱连通的有向无环生成子图，使得每个根强连通分量恰好有一个源顶点，并为这些弧赋正权使其余顶点的加权入度两两不同；随后将原图剩余弧赋予一个共同的足够小的正权。通过扰动分析保证非零特征值两两不同，同时零特征值半单，重数等于根强连通分量数。作者还基于判别式给出了共同弧权可取值区间的计算准则。

Result: 该构造方法保证所得拉普拉斯矩阵具有完整的特征向量集，即存在N个线性无关的特征向量，从而恢复了MSF变分方程的完全解耦形式，使MSF分析能够适用于一般的弱连通有向网络。

Conclusion: 通过合理的弧加权方案，可以克服有向网络拉普拉斯矩阵不可对角化的障碍，使得主稳定函数方法得以推广到更广泛的网络拓扑，并为实际应用中共同弧权的选取提供了判别式准则。
...
### [49] [Optimal Scheduling of Road Maintenance Jobs Considering Impact on Traffic Flows](https://arxiv.org/abs/2608.14491)
*Charitha Nandepu,Lohitha Kalepu,Gabriele Ciavarella,SangWoo Park*

Main category: eess.SY

TL;DR: 本文研究了用数据驱动代理模型直接近似均衡弧流量，以替代重复求解均衡交通分配模型，从而支持网络级养护规划的优化调度。基于新泽西州纽瓦克地区的真实交通数据验证了方法的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 网络级养护规划需要在道路能力下降的情况下反复评估均衡交通流，而均衡交通分配模型的重复求解在计算上代价高昂，难以嵌入养护调度优化问题中，因此亟需高效的替代方案。

Method: 提出数据驱动的代理模型，直接从起讫点（OD）需求近似均衡弧流量，以基于优化的均衡解作为地面真值进行训练和验证。

Result: 基于新泽西纽瓦克地区的真实交通数据案例研究表明，所提出的代理模型方法能够有效逼近均衡弧流量，并具备良好的可扩展性，可作为未来养护调度框架的构建模块。

Conclusion: 数据驱动代理模型是替代重复均衡交通分配计算的可行且可扩展的方案，为将交通流评估嵌入网络级养护调度优化问题提供了有效支撑。
